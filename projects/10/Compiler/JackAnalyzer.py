...
class Analyze:
    ...
    ## ! This is very lazy programming, most of this code is copy-pasted from 08/VMTranslater/main.pys
    def _file_handler(self, file_from: str | Path) -> Tuple[list[Path], Path]:
        ...
        #if file_to == "none": file_to = (file_from.parent if is_file else file_from) / (name.strip('.jack') + ".xml")
    ...
    def main(self, file_from: Path | str):
        ...
        for file in files:
            ...
            with open(file, "r") as f, open(dir_to / "MyXml" / (file.name.strip('.jack') + ".xml"), "w") as output:
