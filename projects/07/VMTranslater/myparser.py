...
class Parser:
    def parse(self, file: str) -> list[Command]:
        ...
        return [self.single(line) for line in lines]

    def single(self, line: str) -> Command:
        values = line.split(" ")
        if len(values) == 1:
            return self.lenOne(values[0])
        elif len(values) == 2:
            return self.lenTwo(values[0], values[1])
        elif len(values) == 3:
            return self.lenThree(values[0], values[1], int(values[2]))
